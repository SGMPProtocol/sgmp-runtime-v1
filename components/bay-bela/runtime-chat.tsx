  ...
  async function handleSend() {
    ...
    if (result.success && result.assistantMessage) {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Bağlantı kesildi. Birazdan tekrar dene.",
        },
      ]);
    }
    ...
    <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 md:p-8 flex flex-col h-[600px]">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`w-2 h-2 rounded-full ${isInitialized ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]" : "bg-amber-500"} ${isLoading ? "animate-pulse" : ""}`}
          />
          <span className="text-xs text-[hsl(var(--muted-foreground))]">
            {isLoading ? "Thinking..." : isInitialized ? "Active" : "Connecting..."}
          </span>
        </div>
      </div>
